...
class Cnv:
    ...
    def clean_deletions_and_low_abunds(
        ...
        self.client_gene_reads = self.remove_all_table_entries_by_correction_matrix(
            z_1_mat,
    ...
    def remove_genes_with_same_uniref_in_same_client(self):
        df_uniq = self.client_gene_reads.reset_index()\
            .set_index('species')\
            .groupby('uniref90', as_index=False)\
            .count()
        
        df_map = self.client_gene_reads.reset_index()[['species', 'uniref90']]
        
        df_uniq = pd.merge(df_uniq, df_map, on='uniref90')
        df_uniq = df_uniq.set_index(['species', 'uniref90'])
    ...
    def sort_genes_by_cnv(
        ...
        self.client_gene_reads = pd.DataFrame(
            self.client_gene_reads.as_matrix() * np.atleast_2d(self.gene_lengths).T,
            columns=self.client_gene_reads.columns,
            index=self.client_gene_reads.index)\
            .astype('float32')
    ...
    def sort_genes_by_deletions(
        ...
        # Tmp tali - why do they need to have the same shape?
        if not self.check_same_genes_and_clients():
    ...
    def set_nan_deletions_as_zero(self):
        ...
        gs_mat = self.client_gene_reads.as_matrix()
    ...
    def remove_genes_with_low_clients_num(self, min_num_of_client_for_gene):
        ...
        self.client_gene_reads = self.client_gene_reads[
            self.client_gene_reads.count(1) > min_num_of_client_for_gene]
    ...
    def set_cn_table(self, min_num_of_client_for_gene, max_possible_copy_number):
        cn_mat = self.client_gene_reads.as_matrix().copy()
        cn_mat[~np.isnan(self.client_gene_reads.as_matrix())] = 1
        ...
        df_expected_std = df_expected_std.astype('float32')
        best_z_mat = abs(
            (self.client_gene_reads.as_matrix() - df_expected_means.as_matrix())
    ...
    def set_expected_num_reads_per_client(
        ...
        lst_exp_reads = []
        df_exp_reads = (self.client_gene_reads / (self.df_abund * self.df_nr)).astype('float32')
    ...
    def check_same_genes_and_clients(self):
        res = True
        res = res and all(self.client_gene_reads.columns == self.df_abund.columns)
        res = res and all(self.df_abund.columns == self.df_nr.columns)
        res = res and all(self.client_gene_reads.index == self.df_abund.index)
    ...
    def remove_short_genes_and_get_lengths_vector(self, min_length):
        self.client_gene_reads, self.gene_lengths = self.remove_short_genes_for_df(
            min_length,
            self.client_gene_reads)
    ...
    def remove_short_genes_for_df(self, min_length, df):
        ...
        df = df.reset_index()
        df = pd.merge(df, self.df_length, on=['species', 'uniref90'])
        df = df[df.length > min_length]
        
        gene_lengths = np.array(df.length)
        df = df.drop(['length'], 1)
        df = df.set_index(['species', 'uniref90'])
