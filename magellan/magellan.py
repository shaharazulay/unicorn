...
class Cnv:
    ...
    def set_deletions_table_and_clean_low_abunds(self,min_abund,max_0_zscore,max_deletion_zscore):
        ...
        z_1_mat=(self.df_genes_spec.as_matrix()-df_expected_means.as_matrix())/df_expected_std.as_matrix()
        ...
        self.df_del_or_not = pd.DataFrame(del_or_not_mat,columns=self.df_genes_spec.columns,index=self.df_genes_spec.index)
        
        
    def clean_deletions_and_low_abunds(self,min_abund,max_0_zscore,max_deletion_zscore):
        df_abund_mat = self.df_abund.as_matrix()
        df_expected_means = self.df_k*self.df_nr*1
        df_expected_means = df_expected_means.astype('float32')
        df_expected_std = np.sqrt(self.df_k*self.df_nr*1*(1-self.df_k*1))
        ...
        self.df_genes_spec=self.remove_all_table_entries_by_correction_matrix(z_1_mat,self.df_genes_spec)
        self.df_abund=self.remove_all_table_entries_by_correction_matrix(z_1_mat,self.df_abund)
        self.df_nr=self.remove_all_table_entries_by_correction_matrix(z_1_mat,self.df_nr)
        
    def remove_genes_with_same_uniref_in_same_client(self):
        df_uniq = self.df_genes_spec.reset_index().set_index('specie').groupby('uniref90',as_index=False).count()
        ...
        self.df_genes_spec=self.remove_all_table_entries_by_correction_matrix(df_uniq_mat,self.df_genes_spec)
        self.df_abund=self.remove_all_table_entries_by_correction_matrix(df_uniq_mat,self.df_abund)
        self.df_nr=self.remove_all_table_entries_by_correction_matrix(df_uniq_mat,self.df_nr)
        
    
    def sort_genes_by_cnv(self,
                          output_path,
        ...
        self.set_expected_num_reads_per_client(max_num_reads_for_search_exp_reads,
        ...
        self.df_genes_spec =pd.DataFrame(
            ...
            index=self.df_genes_spec.index).astype('float32')
        self.clean_deletions_and_low_abunds(min_abund,max_0_zscore,max_deletion_zscore)
        self.remove_genes_with_same_uniref_in_same_client()
        self.set_table_of_k()
        self.remove_genes_with_low_clients_num(min_num_of_client_for_gene)
        self.set_cn_table(min_num_of_client_for_gene,max_possible_copy_number)
        self.sort_cn_table_by_second()
        self.df_cn.to_pickle(output_path)
    
    def sort_genes_by_deletions(self,
                                output_path,
                                min_gene_length=700.0,
                                max_num_reads_for_search_exp_reads=0.0002,
                                num_bins_for_search_exp_reads = 50,
        ...
        self.set_deletions_table_and_clean_low_abunds(min_abund,max_0_zscore,max_deletion_zscore)
        self.df_del_or_not = self.df_del_or_not[self.df_del_or_not.count(1)>min_num_of_client_for_gene]
    ...
    def set_nan_deletions_as_zero(self):
        ...
        self.df_genes_spec = pd.DataFrame(gs_mat,columns=self.df_genes_spec.columns,index=self.df_genes_spec.index)
    
    def remove_genes_with_low_clients_num(self,min_num_of_client_for_gene):
        self.df_genes_spec = self.df_genes_spec[self.df_genes_spec.count(1)>min_num_of_client_for_gene]
        self.df_abund = self.df_abund[self.df_abund.count(1)>min_num_of_client_for_gene]
    ...
    def set_cn_table(self,min_num_of_client_for_gene,max_possible_copy_number):
        ...
        df_expected_std = np.sqrt(self.df_k*self.df_nr*1*(1-self.df_k*1))
        df_expected_std = df_expected_std.astype('float32')
        best_z_mat = abs((self.df_genes_spec.as_matrix()-df_expected_means.as_matrix())/df_expected_std.as_matrix())
        ...
        for i in range(1,max_possible_copy_number):
            ...
            df_expected_std = df_expected_std.astype('float32')
            curr_z_mat = abs((self.df_genes_spec.as_matrix()-df_expected_means.as_matrix())/df_expected_std.as_matrix())
            cn_mat[best_z_mat>curr_z_mat]=i
            best_z_mat[best_z_mat>curr_z_mat]=curr_z_mat[best_z_mat>curr_z_mat]
        self.df_cn = pd.DataFrame(cn_mat,columns=self.df_genes_spec.columns,index = self.df_genes_spec.index)
        
       

    def set_expected_num_reads_per_client(self,max_num_reads_for_search,num_bins_for_search_exp_reads,start_bin_search_exp_reads):
        lst_exp_reads=[]
        ...
        for i in df_exp_reads.columns:
            x = np.histogram(
                ...
                num_bins_for_search_exp_reads)
            lst_exp_reads.append(x[1][x[0][start_bin_search_exp_reads:x[0].shape[0]].argmax()+start_bin_search_exp_reads])
    ...
    def drop_duplicate_uniref_in_the_same_bacteria(self):
        self.df_genes_spec = self.df_genes_spec.reset_index().drop_duplicates(['specie','uniref90'],keep = False).set_index(['specie','uniref90'])
        self.df_abund = self.df_abund.reset_index().drop_duplicates(['specie','uniref90'],keep = False).set_index(['specie','uniref90'])
        self.df_nr = self.df_nr.reset_index().drop_duplicates(['specie','uniref90'],keep = False).set_index(['specie','uniref90'])
        self.df_length = self.df_length.drop_duplicates(['specie','uniref90'],keep = False)

        
    def remove_short_genes_and_get_lengths_vector(self,min_length):
        self.df_genes_spec,self.gene_lengths = self.remove_short_genes_for_df(min_length,self.df_genes_spec)
        self.df_abund,self.gene_lengths = self.remove_short_genes_for_df(min_length,self.df_abund)
        self.df_nr,self.gene_lengths = self.remove_short_genes_for_df(min_length,self.df_nr)
    ...
    def sort_cn_table_by_smalest_window(self,max_frac,min_frac,interval_frac,num_of_bins,outliers_perc):
        ...
        def sort_cn_table_by_second(self):
            self.df_cn['sec'] = [self.get_second(list( self.df_cn.iloc[i])) for i in range(self.df_cn.shape[0])]
...
if __name__ == '__main__':
	...
	parser.add_argument('--df_gene_spec_path',dest ='df_gene_spec_path',required='True',help='the path of the reads dataframe')
	parser.add_argument('--df_abund_path',dest ='df_abund_path',required='True',help='the path of the specie abandance dataframe')
	parser.add_argument('--df_nr_path',dest ='df_nr_path',required='True',help='the path of the total_reads dataframe')
	parser.add_argument('--df_length_path',dest ='df_length_path',required='True',help='the path of the gene_length dataframe')
	parser.add_argument('--output_path',dest ='output_path',required='True',help='the path of the output dataframe(pickle)')
        parser.add_argument('--deletions',dest ='deletions',action='store_true',default=False,help='if stated, the genes will be sorted by deletions')
        parser.add_argument('--min_gene_length',dest ='min_gene_length',type=int ,default=700,help='genes shorter than this value will be ignored')
	parser.add_argument('--max_num_reads_for_search_exp_reads',
			    dest ='max_num_reads_for_search_exp_reads',
			    type=float ,
			    default=0.0002,
			    help='max num of reads to look at when searching expected reads per client')
	parser.add_argument('--num_bins_for_search_exp_reads',
			     dest ='num_bins_for_search_exp_reads',
			     type=int ,default=50,
			     help='num of bins to use when searching for expected reads per client')
	parser.add_argument('--start_bin_search_exp_reads',
			    dest ='start_bin_search_exp_reads',
			    type=int ,
			    default=10,
			    help='bin to start from when searching for expected reads per client')
	parser.add_argument('--min_abund',dest ='min_abund',type=float ,default=0.004,help='species with lower abundance will be ignored')
	parser.add_argument('--max_0_zscore',dest ='max_0_zscore',type=int ,default=-2,help='if 0 gets a z score higher than this value, the gene will be ignored')
	parser.add_argument('--max_deletion_zscore',dest ='max_deletion_zscore',type=int ,default=-2,help='z scores below this value will be considered as deletions')
	parser.add_argument('--min_num_of_client_for_gene',
			    dest ='min_num_of_client_for_gene',
			    type=int ,default=67,
			    help='gene that appear in less then this number of clients will be ignored')
        parser.add_argument('--max_possible_copy_number',
			    dest ='max_possible_copy_number',
			    type=int ,
			    default=60,
			    help='max copy number considered (above this value it will be considered this value)')
	parser.add_argument('--max_frac_for_sort',dest ='max_frac_for_sort',type=float ,default=0.95,help='the highst fraction of data for smallest window sort')
	parser.add_argument('--min_frac_for_sort',dest ='min_frac_for_sort',type=float ,default=0.5,help='the lowest fraction of data for smallest window sort')
	parser.add_argument('--interval_frac_for_sort',
			   dest ='interval_frac_for_sort',
			   type=float ,
			   default=0.05,
			   help='the interval between fractions of data for smallest window sort')
        parser.add_argument('--outliers_perc_for_sort',
			    dest ='outliers_perc_for_sort',
			    type=float ,
			    default=2.5,
			    help='outlier percent to remove from each size for window sort')         
	parser.add_argument('--num_of_bins_for_sort',dest ='num_of_bins_for_sort',type=int ,default=50,help='number of bins for smallest window sort')
       
	args = parser.parse_args()

        cnv = Cnv(args.df_gene_spec_path,args.df_abund_path,args.df_nr_path,args.df_length_path)
        
	if args.deletions:
		cnv.sort_genes_by_deletions(args.output_path,
		...
		cnv.sort_genes_by_cnv(args.output_path,
                          args.min_gene_length,
                          args.max_num_reads_for_search_exp_reads,
                          args.num_bins_for_search_exp_reads,
                          args.start_bin_search_exp_reads,
